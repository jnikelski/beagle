...
cta_subdir <- paste(analysis_type, scan_id, sep="-")
...
\chapter{Findings: \texttt{CTA-\Sexpr{scan_id}}}
In this chapter, we present the cortical thickness analysis findings for subject \texttt{\Sexpr{keyname}}, based on the T1 scan acquired on \texttt{\Sexpr{scan_id}}. 
