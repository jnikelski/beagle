...
# write out the to-be-printed table as a csv file (set scipen > 0, as we don't want scientific notion)
...
write.csv(print_table.df, file = csvFilename_out_fullPath, row.names = FALSE,)
...
write.csv(neoCortical.df, file = csvFilename_out_fullPath, row.names = FALSE,)
#
...
write.csv(xNeoCortical.df, file = csvFilename_out_fullPath, row.names = FALSE,)
...
write.csv(cerebellar.df, file = csvFilename_out_fullPath, row.names = FALSE,)
