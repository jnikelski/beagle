...
csvFilename_out_fullPath <- file.path(pib_ratios_subdir_fullPath, "ReportTable_PiB_SUVR_Values.csv")
write.csv(suvr.df, file = csvFilename_out_fullPath, row.names = FALSE,)
...
# sort by ratio value and write out as a csv file
ratiosX.df <- ratiosX.df[order(ratiosX.df$ratio, decreasing=TRUE),]
options(scipen=1, digits=8)
csvFilename_out_fullPath <- file.path(pib_ratios_subdir_fullPath, "ReportTable_PiB_Ratio_Values_Sorted_in_Descending_Order.csv")
write.csv(ratiosX.df, file = csvFilename_out_fullPath, row.names = FALSE,)
