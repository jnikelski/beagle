...
# (4) scan_id           [ YYYYMMDD ... must match subdir suffix]
...
pib_subdir <- paste(analysis_type, scan_id, sep="-")
...
# read the scan id for the associated anatomy from the "civet_scan_id.txt" file
...
civet_scan_id <- read.csv(civet_scanId_filename_fullpath, header=FALSE, stringsAsFactors=FALSE)[1,1]
# now create a pleasing full subdir path
masks_dirname <- paste('MASKS', civet_scan_id, sep="-")
...
\chapter{Findings: \texttt{PiB-\Sexpr{scan_id}}}
In this chapter, we present the findings of the amyloid imaging analysis of subject \texttt{\Sexpr{keyname}}, based on the dynamic scan acquired on \texttt{\Sexpr{scan_id}}. Subject \texttt{\Sexpr{keyname}'s} scan resulted in the acquisition of \textbf{\Sexpr{nFrames}} frames.
